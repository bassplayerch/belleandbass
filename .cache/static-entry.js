...
export default (pagePath, callback) => {
  let bodyHtml = ``;
  let headComponents = [
    <meta name="generator" content={`Gatsby ${gatsbyVersion}`} key={`generator-${gatsbyVersion}`} />,
  ];
  let htmlAttributes = {};
  let bodyAttributes = {};
  ...
  scripts
    ...
    .forEach(script => {
      ...
      headComponents.push(
        <link as="script" rel={script.rel} key={script.name} href={`${__PATH_PREFIX__}/${script.name}`} />
      );
    });

  if (page.jsonName in dataPaths) {
    const dataPath = `${__PATH_PREFIX__}/static/d/${dataPaths[page.jsonName]}.json`;
    headComponents.push(<link as="fetch" rel="preload" key={dataPath} href={dataPath} crossOrigin="use-credentials" />);
  }
  ...
  styles
    ...
    .forEach(style => {
      ...
      if (style.rel === `prefetch`) {
        headComponents.push(
          <link as="style" rel={style.rel} key={style.name} href={`${__PATH_PREFIX__}/${style.name}`} />
        );
      } else {
  ...
  const windowData = `/*<![CDATA[*/window.page=${JSON.stringify(page)};${
    page.jsonName in dataPaths ? `window.dataPath="${dataPaths[page.jsonName]}";` : ``
  }/*]]>*/`;
