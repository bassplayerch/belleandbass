...
apiRunnerAsync(`onClientEntry`).then(() => {
  ...
  if (
    ...
    (!page.matchPath || !match(__PATH_PREFIX__ + page.matchPath, browserLoc.pathname)) &&
    ...
    navigate(__PATH_PREFIX__ + page.path + browserLoc.search + browserLoc.hash, { replace: true });
  }
  ...
  loader.getResourcesForPathname(browserLoc.pathname).then(() => {
    ...
    const WrappedRoot = apiRunner(`wrapRootElement`, { element: <Root /> }, <Root />, ({ result }) => {
    ...
    domReady(() => {
      renderer(<NewRoot />, typeof window !== `undefined` ? document.getElementById(`___gatsby`) : void 0, () => {
