  ...
  if (!fetchingPageResourceMapPromise) {
    fetchingPageResourceMapPromise = new Promise(resolve => {
      asyncRequires
        ...
        .catch(e => {
          console.warn(`Failed to fetch pages manifest. Gatsby will reload on next navigation.`);
...
const prefetchResource = resourceName => {
  if (resourceName.slice(0, 12) === `component---`) {
    return Promise.all(createComponentUrls(resourceName).map(url => prefetchHelper(url)));
  } else {
    const url = createJsonURL(jsonDataPaths[resourceName]);
 ...
 */
const shouldFallbackTo404Resources = path => (hasPageResourceMap || inInitialRender) && path !== `/404.html`;
...
const queue = {
  ...
  enqueue: path => {
    if (!apiRunner) console.error(`Run setApiRunnerForLoader() before enqueing paths`);
  ...
  getResourcesForPathname: path =>
    new Promise((resolve, reject) => {
      ...
      if (process.env.NODE_ENV !== `production`) {
        ...
        Promise.all([getResourceModule(page.componentChunkName), getResourceModule(page.jsonName)]).then(
          ([component, json]) => {
            if (!(component && json)) {
              resolve(null);
              return;
